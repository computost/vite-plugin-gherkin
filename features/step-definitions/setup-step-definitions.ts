...
Given("a test context with a number repository", (_, { importTestFrom }) => {
  importTestFrom.testFile = path.resolve(__dirname, "../../tests/test.ts");
});

Given("a Given step that stores a number", (_, { setupFiles }) => {
  setupFiles.push(path.resolve(__dirname, "../../tests/given-store-number.ts"));
});

Given("a When step that adds 2 numbers", (_, { setupFiles }) => {
